<#
.SYNOPSIS
    Local entry point for the real Unreal Engine CI lane (#24).
...
.DESCRIPTION
    ...
    Any failure writes a concise failure_context.txt under the normal CI
    artifact root and rethrows the original error.
#>
[CmdletBinding()]
param(
...
function Write-YacsUnrealFailureContext {
    ...
    try {
        $Files = @(
            Get-ChildItem -LiteralPath $ArtifactRoot -Recurse -File -ErrorAction Stop |
...
try {
    ...
    if (-not $Context.EngineVersion) {
        throw 'Unreal CI preflight did not resolve an engine version.'
    }
    if ($Context.EngineVersion.MajorVersion -ne 5 -or $Context.EngineVersion.MinorVersion -ne 8) {
        throw (
            "Unreal CI requires UE 5.8.x; resolved {0}.{1}.{2}." -f
    ...
    if ([int]$Summary.Failed -ne 0 -or [int]$Summary.Errors -ne 0) {
        throw (
            "Unreal CI summary is not green: failed={0}, errors={1}." -f
            $Summary.Failed,
            $Summary.Errors
        )
    }
    ...
    $CiSummary | ConvertTo-Json -Depth 4 |
        Set-Content -LiteralPath $CiSummaryPath -Encoding UTF8
}
catch {
    $OriginalError = $_
    try {
        Write-YacsUnrealFailureContext -Reason $OriginalError.Exception.Message
    }
    catch {
        Write-Warning (
            'Failed to write YACS Unreal failure context: {0}' -f
            $_.Exception.Message
        )
    }
    throw $OriginalError
